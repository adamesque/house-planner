...
affordability bar, and mobile-friendly dollar steppers.
...
### 4. Shareable state via URL hash

State lives only in `localStorage`, so phone and laptop disagree and scenarios
can't be sent to a partner. Serialize state into the URL fragment (read on
load, written on change) for sync-by-link and "look at this scenario" sharing
with no backend. For a two-person decision tool, possibly the
